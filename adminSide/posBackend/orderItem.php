        ...
        <div class="row">
            <div class="col-md-6 order-md-1 m-1" id="item-select-section ">
                <div class="container-fluid pt-4 pl-500 row" style=" margin-left: 10rem;width: 81% ;">
                    <div class="mt-5 mb-2">
                    <h3 class="pull-left">Bebidas Alimentos</h3>
                    ...
                    <div class="mb-3">
                        <form method="POST" action="#">
                            <div class="row">
                                <div class="col-md-6">
                                    <input type="text" required="" id="search" name="search" class="form-control" placeholder="Pesquisar alimentos e bebidas">
                                </div>
                                <div class="col-md-3">
                                    <button type="submit" class="btn btn-dark">Pesquisa</button>
                                </div>
                                <div class="col" style="text-align: right;" >
                                    <a href="orderItem.php?bill_id=<?php echo $bill_id; ?>&table_id=<?php echo $table_id; ?>" class="btn btn-light">Mostre tudo</a>
                                </div>
                            </div>
                        </form>
                    </div>
                    <div style="max-height: 45rem;overflow-y: auto;">
                        ...
                        if ($result) {
                            if (mysqli_num_rows($result) > 0) {
                                ...
                                echo '<div class="alert alert-danger"><em>Nenhum item de menu foi encontrado.</em></div>';
                            }
                        } else {
                            ...
                            <div style="max-height: 40rem;overflow-y: auto;">
                                ...
                                if ($cart_result && mysqli_num_rows($cart_result) > 0) {
                                    while ($cart_row = mysqli_fetch_assoc($cart_result)) {
                                        ...
                                        echo '<td>R$ ' . number_format($item_price,2) . '</td>';
                                        echo '<td>' . $quantity . '</td>';
    ...
    <table class="table table-bordered ">
        <tbody>
            <tr>
                ...
                <td>R$ <?php echo number_format($cart_total, 2); ?></td>
            </tr>
            <tr>
                <td><strong>Total tributado</strong></td>
                <td>R$ <?php echo number_format($cart_total * $tax, 2); ?></td>
            </tr>
            <tr>
                <td><strong> Total a pagar</strong></td>
                <td>R$ <?php echo number_format(($tax * $cart_total) + $cart_total, 2); ?></td>
            </tr>
        </tbody>
    </table>
</div>
                        ...
                        if ($has_payment_time) {
                            ...
                            echo '<br><a href="receipt.php?bill_id=' . $bill_id . '" class="btn btn-light">
                            Imprimir recibo <span class="fa fa-receipt text-black"></span></a></div>';
                            

                            
                        } elseif(($tax * $cart_total + $cart_total) > 0) {
                            echo '<br><a href="idValidity.php?bill_id=' . $bill_id . '" class="btn btn-success">Pagar conta</a>';
                        } else {
                            echo '<br><h3>Adicione item ao carrinho para continuar</h3>';
                        }
                    ...
                    <?php 
                       echo '<form class="mt-3" action="newCustomer.php" method="get">'; // Add this form element
                        ...
                        echo '<button type="submit" name="new_customer" value="true" class="btn btn-warning">Novo cliente</button>';
                        echo '</form>';
